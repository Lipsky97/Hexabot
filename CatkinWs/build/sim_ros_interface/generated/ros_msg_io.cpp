...
void write__project_demo__Leg(const project_demo::Leg& msg, int stack, const WriteOptions *opt)
{
    try
    {
        simPushTableOntoStackE(stack);
        ...
        {
            // write field 'foot'
            simPushStringOntoStackE(stack, "foot", 0);
            write__float32(msg.foot, stack, opt);
            simInsertDataIntoStackTableE(stack);
        }
        catch(exception& ex)
        {
            std::string msg = "field 'foot': ";
            msg += ex.what();
            throw exception(msg);
        }
        try
        {
            // write field 'knee'
            simPushStringOntoStackE(stack, "knee", 0);
            write__float32(msg.knee, stack, opt);
            simInsertDataIntoStackTableE(stack);
        }
        catch(exception& ex)
        {
            std::string msg = "field 'knee': ";
            msg += ex.what();
            throw exception(msg);
        }
        try
        {
            // write field 'tigh'
            simPushStringOntoStackE(stack, "tigh", 0);
            write__float32(msg.tigh, stack, opt);
            simInsertDataIntoStackTableE(stack);
        }
        catch(exception& ex)
        {
            std::string msg = "field 'tigh': ";
            msg += ex.what();
            throw exception(msg);
        }
    }
    ...
    {
        std::string msg = "write__project_demo__Leg: ";
        msg += ex.what();
        throw exception(msg);
    }
}

void read__project_demo__Leg(int stack, project_demo::Leg *msg, const ReadOptions *opt)
{
    try
    {
        int r = simGetStackTableInfoE(stack, 0);
        ...
        while(numItems >= 1)
        {
            ...
            if((str = simGetStackStringValueE(stack, &strSz)) != NULL && strSz > 0)
            {
                ...
                else if(strcmp(str, "foot") == 0)
                {
                    ...
                    {
                        // read field 'foot'
                        read__float32(stack, &(msg->foot), opt);
                    }
                    catch(exception& ex)
                    {
                        std::string msg = "field foot: ";
                ...
                else if(strcmp(str, "knee") == 0)
                {
                    ...
                    {
                        // read field 'knee'
                        read__float32(stack, &(msg->knee), opt);
                    }
                    catch(exception& ex)
                    {
                        std::string msg = "field knee: ";
                ...
                else if(strcmp(str, "tigh") == 0)
                {
                    try
                    {
                        // read field 'tigh'
                        read__float32(stack, &(msg->tigh), opt);
                    }
                    catch(exception& ex)
                    {
                        std::string msg = "field tigh: ";
    ...
    {
        std::string msg = "read__project_demo__Leg: ";
        msg += ex.what();
        throw exception(msg);
    }
}
...
void ros_callback__project_demo__Leg(const boost::shared_ptr<project_demo::Leg const>& msg, SubscriberProxy *proxy)
{
    int stack = -1;
    try
    {
        stack = simCreateStackE();
        write__project_demo__Leg(*msg, stack, &(proxy->wr_opt));
        simCallScriptFunctionExE(proxy->topicCallback.scriptId, proxy->topicCallback.name.c_str(), stack);
        simReleaseStackE(stack);
        stack = -1;
    }
    catch(exception& ex)
    {
        if(stack != -1)
            simReleaseStack(stack); // don't throw
        std::string msg = "ros_callback__project_demo__Leg: ";
        msg += ex.what();
        simSetLastError(proxy->topicCallback.name.c_str(), msg.c_str());
    }
}
